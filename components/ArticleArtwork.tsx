...
type Props={article:Pick<Article,"slug"|"category"|"categoryLabel"|"title"|"featuredImage">;compact?:boolean};
...
export function ArticleArtwork({article,compact=false}:Props){
 if(article.featuredImage)return <div className={`article-artwork article-featured-image${compact?" is-compact":""}`} role="img" aria-label={article.featuredImage.altText??article.title}><img src={article.featuredImage.url} alt={article.featuredImage.altText??article.title}/></div>;
