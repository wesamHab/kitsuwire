import { Bot, BrainCircuit, ChartCandlestick, Cloud, Coins, Container, Cpu, Database, GitBranch, Github, Globe2, Network, ServerCog, Sparkles, TrendingUp } from "lucide-react";
...
const TopicIcon=({kind}:{kind:string})=>{
 const p={size:48,strokeWidth:1.7};
 switch(kind){
  case"kubernetes":return <KubernetesMark/>;
  case"docker":return <DockerMark/>;
  case"terraform":return <TerraformMark/>;
  case"github":return <Github {...p}/>;
  case"git":return <GitBranch {...p}/>;
  case"api":return <ApiMark/>;
  case"devops":return <ServerCog {...p}/>;
  case"microservices":return <Network {...p}/>;
  case"containers":return <Container {...p}/>;
  case"datacenter":return <Database {...p}/>;
  case"cloud":return <Cloud {...p}/>;
  case"network":return <Globe2 {...p}/>;
  case"gold":return <Coins {...p}/>;
  case"etf":return <ChartCandlestick {...p}/>;
  case"inflation":return <TrendingUp {...p}/>;
...
function Scene({kind}:{kind:string}){
 if(kind==="kubernetes")return <><div className="scene-node n1">POD</div><div className="scene-node n2">POD</div><div className="scene-node n3">POD</div><div className="scene-line l1"/><div className="scene-line l2"/><div className="scene-line l3"/></>;
 if(kind==="docker"||kind==="containers")return <><div className="scene-box b1"/><div className="scene-box b2"/><div className="scene-box b3"/><div className="scene-box b4"/></>;
 if(kind==="terraform")return <><div className="scene-cube c1"/><div className="scene-cube c2"/><div className="scene-cube c3"/></>;
 if(kind==="git"||kind==="github"||kind==="devops")return <><div className="scene-branch"><span/><span/><span/></div><div className="scene-code">01<br/>10<br/>01</div></>;
 if(kind==="api"||kind==="microservices"||kind==="network")return <><div className="scene-service s1"/><div className="scene-service s2"/><div className="scene-service s3"/><div className="scene-service s4"/><div className="scene-connector x1"/><div className="scene-connector x2"/></>;
 if(kind==="datacenter"||kind==="cloud"||kind==="chip")return <><div className="scene-rack r1"/><div className="scene-rack r2"/><div className="scene-rack r3"/><div className="scene-light"/></>;
 if(kind==="gold")return <><div className="scene-gold g1"/><div className="scene-gold g2"/><div className="scene-gold g3"/></>;
 if(kind==="etf")return <><div className="scene-chart"><i/><i/><i/><i/></div><div className="scene-pie"/></>;
 if(kind==="inflation"||kind==="rates"||kind==="markets")return <><div className="scene-chart"><i/><i/><i/><i/><i/></div><div className="scene-arrow"/></>;
 if(kind==="agents"||kind==="llm")return <><div className="scene-ai-core"/><div className="scene-ai-dot d1"/><div className="scene-ai-dot d2"/><div className="scene-ai-dot d3"/></>;
 return <div className="scene-default"/>;
}

const labelFor=(kind:string)=>({kubernetes:"Kubernetes",docker:"Docker",terraform:"Terraform",github:"GitHub",git:"Git",api:"API",devops:"DevOps",microservices:"Microservices",containers:"Containers",datacenter:"Data Center",cloud:"Cloud",network:"Network",gold:"Gold",etf:"ETF",inflation:"Inflation",rates:"Rates",markets:"Markets",agents:"AI Agents",llm:"Generative AI",chip:"Compute"}[kind]??"KitsuWire");

export function ArticleArtwork({article,compact=false}:Props){
 const kind=artworkKind(article.slug);
 return <div className={`article-artwork artwork-${article.category} artwork-${kind}${compact?" is-compact":""}`} role="img" aria-label={`${labelFor(kind)} illustration for ${article.title}`}>
   <div className="art-grid"/><div className="art-glow"/><Scene kind={kind}/>
   <div className="art-icon"><TopicIcon kind={kind}/></div>
   <div className="art-label"><span>{article.categoryLabel}</span><strong>{labelFor(kind)}</strong></div>
 </div>
}
