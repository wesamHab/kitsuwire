...
export async function generateMetadata({params}:{params:Promise<{category:string}>}):Promise<Metadata>{const{category}=await params;if(!categories.includes(category as Category))return{};const meta=await getCategoryMeta(category as Category);return pageMetadata({title:meta.title,description:meta.description,path:`/category/${category}`})}
